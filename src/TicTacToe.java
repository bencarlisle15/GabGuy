...
public class TicTacToe extends JPanel implements MouseListener {
    ...
    private void paintLetter(Graphics g, int r, int c) {
        ...
        // draws an x
            ...
            g2.drawLine(lineOneX1 + addRow, lineOneY1 + addCol, lineOneX2 + addRow,
                                    lineOneY2 + addCol);
            g2.drawLine(lineTwoX1 + addRow, lineTwoY1 + addCol, lineTwoX2 + addRow,
                                    lineTwoY2 + addCol);
        }
    }
    ...
    public void paintOrginal(Graphics g) {
        ...
        g.fillRoundRect(padding + 2 * space + width, padding, width, height, round,
                                        round);
        g.fillRoundRect(padding, padding + space, height, width, round, round);
        g.fillRoundRect(padding, padding + 2 * space + width, height, width, round,
                                        round);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        // click in dimensions for each square sets the square in the 2d array to x
        // and goes to the comp turn
        else if (e.getX() >= oneStart && e.getX() < oneEnd &&
                         e.getY() >= oneStart && e.getY() < oneEnd && board[0][0] == 0) {
            board[0][0] = 'X';
            nextMove();
        } else if (e.getX() >= twoStart && e.getX() < twoEnd &&
                             e.getY() >= oneStart && e.getY() < oneEnd && board[0][1] == 0) {
            board[0][1] = 'X';
            nextMove();
        } else if (e.getX() >= threeStart && e.getX() < threeEnd &&
                             e.getY() >= oneStart && e.getY() < oneEnd && board[0][2] == 0) {
            System.out.println("box3");
            board[0][2] = 'X';
            nextMove();
        } else if (e.getX() >= oneStart && e.getX() < oneEnd &&
                             e.getY() >= twoStart && e.getY() < twoEnd && board[1][0] == 0) {
            board[1][0] = 'X';
            nextMove();
        } else if (e.getX() >= twoStart && e.getX() < twoEnd &&
                             e.getY() >= twoStart && e.getY() < twoEnd && board[1][1] == 0) {
            board[1][1] = 'X';
            nextMove();
        } else if (e.getX() >= threeStart && e.getX() < threeEnd &&
                             e.getY() >= twoStart && e.getY() < twoEnd && board[1][2] == 0) {
            board[1][2] = 'X';
            nextMove();
        } else if (e.getX() >= oneStart && e.getX() < oneEnd &&
                             e.getY() >= threeStart && e.getY() < threeEnd &&
                             board[2][0] == 0) {
            board[2][0] = 'X';
            nextMove();
        } else if (e.getX() >= twoStart && e.getX() < twoEnd &&
    ...
    private void checkWin() {
        // types of win scenarios and send the positions of the first and last
        // points of the row
    ...
    private void gameOver(int r1, int c1, int r2, int c2) {
        // identifies who won and where the coordinates of the first and last points
        // are and that the game is over
        done = true;
        if (board[r1][c1] == 'X')
            win = true;
        if (c1 == 0)
    ...
    private void nextMove() {
        // makes sure that there are less os than xs so that cpu cannot make two
        // moves
        int x = 0;
        int o = 0;
        for (int r = 0; r < board.length; r++)
        ...
        // if no two in a rows are present then pick one of the five optimal
        // positions
        ...
        // checks to see if win or draw status has changed and repaints to add the
        // new char
        checkWin();
        repaint();
        if (fullBoard()) {
    ...
    private void gameIsOver() {
        ...
        // sets status in the top middle of the frame and as nonfocusable or
        // editable with a mouselistener background
        SimpleAttributeSet center = new SimpleAttributeSet();
        StyleConstants.setAlignment(center, StyleConstants.ALIGN_CENTER);
        status.getStyledDocument().setParagraphAttributes(
                0, status.getStyledDocument().getLength(), center, false);
