 ...
 */
public class CardBoardGUI extends JFrame implements ActionListener {
   ...
   */
  public CardBoardGUI(CardBoard gameBoard) {
    board = gameBoard;
  ...
  private void initDisplay() {
    ...
    try {
      this.setIconImage(
          ImageIO.read(getClass().getResource("cards/acespadesS.png")));
    ...
    for (int k = 0; k < board.size(); k++) {
      ...
      displayCards[k].setBounds(cardCoords[k].x, cardCoords[k].y, CARD_WIDTH,
                                CARD_HEIGHT);
    ...
    restartButton.setBounds(BUTTON_LEFT, BUTTON_TOP + BUTTON_HEIGHT_INC, 100,
                            30);
    ...
    winMsg = new JLabel();
    winMsg.setBounds(LABEL_LEFT, LABEL_TOP + LABEL_HEIGHT_INC, 225, 30);
    winMsg.setFont(new Font("SansSerif", Font.BOLD, 12));
    winMsg.setForeground(Color.BLUE);
    winMsg.setText("You Win! Click anywhere to exit.");
    panel.add(winMsg);
    winMsg.setVisible(false);

    lossMsg = new JLabel();
    lossMsg.setBounds(LABEL_LEFT, LABEL_TOP + LABEL_HEIGHT_INC, 225, 30);
    lossMsg.setFont(new Font("SanSerif", Font.BOLD, 12));
    lossMsg.setForeground(Color.RED);
    lossMsg.setText("Sorry, You Lose. Click anywhere to exit.");
    panel.add(lossMsg);
  ...
  private void signalWin() {
    ...
    panel.addMouseListener(new MyMouseListener(true));
  }
  ...
  private class MyMouseListener implements MouseListener {
    ...
    public MyMouseListener(boolean q) { quit = q; }
