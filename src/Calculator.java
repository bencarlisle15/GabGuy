...
private String[] operatorChars={"^","x","*","/","+","-"};
	...
	public boolean checkEq(String eq)
	{
		...
		String oper="(\\^|\\+|\\-|\\*|x|\\/)";
		String num="(([0-9]+)(\\.?)([0-9]*))";
		if (Pattern.compile("("+num+"("+oper+num +")*)").matcher(eq).matches())
			return true;
		return false;
	}
	...
	public String equationSolver(String equat)
	{
		...
		//adds the chars from the last pos to the position to the terms ArrayList and the next char to the terms one
		...
		for (int i=0;i<operatorChars.length;i++)
			for (int t=0;t<operators.size();t++)
				if (operatorChars[i].equals(operators.get(t)))
					{
					//sets the term at the same pos as the operator equal to the operation between that and the second term
						terms.set(t,twoTerms(operators.get(t),terms.get(t),terms.get(t+1)));
						//removes the next term and removes the operator at the current pos and subsequently t by one
						terms.remove(t+1);
						operators.remove(t);
						t--;
					}
