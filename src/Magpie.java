...
public class Magpie extends Thread {
  ...
  public void run() {
    ...
    if (code == 1) {
      // looks for game
      if (statement.contains("rock") || statement.contains("paper") ||
          statement.contains("scissors")) {
      ...
      else if (statement.contains("tic") || statement.contains("tac") ||
               statement.contains("toe")) {
        // starts TTT
    ...
    // if the user hit f1
      ...
      if (statement.indexOf("n") >= 0)
        ...
        Scanner is =
            new Scanner(getClass().getResourceAsStream("files/help.md"));
        String ans = "";
      ...
      if (others.containsKey(statement))
        ...
        if (list.containsKey(
                statement.substring(statement.indexOf("what's on") + 10)))
          response += "Your list, " +
                      statement.substring(statement.indexOf("what's on") + 10) +
                      ", has:\n";
        ...
        response = new SimpleDateFormat("HH:mm:ss")
                       .format(Calendar.getInstance().getTime());
      else if (findKeyword(statement, "date") || findKeyword(statement, "day"))
        response = new SimpleDateFormat("MM/dd/yyyy")
                       .format(Calendar.getInstance().getTime());
      else if (statement.matches("(.*)i (([a-zA-Z]| )+) you"))
        response = "Why do you " +
                   statement.substring((statement.indexOf("i ") + 2),
                                       statement.length() - 4) +
                   " me";
      else if (findKeyword(statement, "I want to"))
        response =
            "Why do you want to " +
            statement.substring(Math.min(statement.indexOf("I want to") + 11,
                                         statement.length()));
        ...
        response = "Prove it";
      else if (statement.substring(0, Math.min(4, statement.length()))
                   .equals("ping"))
        response = ping();
        ...
        response = "Why would I do " +
                   statement.substring(statement.indexOf("do") + 3);
      else
  ...
  private boolean findKeyword(String phrase, String goal) {
    ...
    return phrase.matches("([^a-zA-Z]|\\A)" + goal.toLowerCase() +
                          "(\\z|[^a-zA-Z])");
  }

  private String addResponse(String s) {
    // checks if the key already exists
    if (!others.containsKey(s.substring(s.indexOf("if I say") + 10,
                                        s.indexOf("then say") - 1))) {
      // adds the key and response to the other statements list
      others.put(
          s.substring(s.indexOf("if I say") + 10, s.indexOf("then say") - 1),
          s.substring(s.indexOf("then say") + 9));
  ...
  private String addToList(String s) {
    ...
    if (list.containsKey(s.substring(s.indexOf("to") + 3)))
      list.get(s.substring(s.indexOf("to") + 3))
          .add(s.substring(s.indexOf("add") + 4, s.indexOf("to") - 1));
    else {
  ...
  private String name(String statement) {
    ...
    name = statement
               .substring(statement.indexOf("my name is ") + 11,
                          statement.indexOf("my name is ") + 12)
               .toUpperCase();
    // sets every letter after a space, dash, or period to uppercase and
    // everything else to lowercase
    for (int i = statement.indexOf("my name is ") + 13; i <= statement.length();
         i++)
      if ((statement.substring(i - 1, i).equals(" ") ||
           statement.substring(i - 1, i).equals(".") ||
           statement.substring(i - 1, i).equals("-")) &&
          i < statement.length()) {
        name += statement.substring(i - 1, i) +
                statement.substring(i, i + 1).toUpperCase();
        i++;
      } else
