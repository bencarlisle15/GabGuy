...
public class Blackjack {
  ...
  private String case0() { return questionDesire(true); }
  ...
  private String case3(String move) {
    ...
    if (move.equals("fold"))
      ...
      // subtracts one from the code so that case3 will be repeated on the next
      // turn
      code--;
      // asks the user what they want to do
      return response + questionMove();
    } else if (move.equals("double down")) {
      // can only double down if it's their first move and they have enough
      // money to double their bet
  ...
  private String case4() {
    ...
    if (hasAce && cardTotal(hand) + 10 <= 21) {
      // creates a fake hand for the purposes of simulating the 11 power of an
      // ace and adds a card of no suit worth ten
    ...
    while (cardTotal(cpuHand) < 21) {
      // only adds if the hand is less than 17 (general rule of thumb number for
      // blackjack)
    ...
    if (cardTotal == 21 && cpuCardTotal != 21 ||
      ...
      // tells the user they have won and adds their bet plus the cpu bet to
      // their ammount
      response += win() + "\n";
      amount += (2 * bet);
    }
    // tie scenarios
    else if (cpuCardTotal == cardTotal || cpuCardTotal > 21 && cardTotal > 21) {
    ...
    if (user) {
      ...
      if (hasAce) {
        if (cardTotal(hand) < 11)
          return "You have a total of " + cardTotal(hand) + " or " +
              (cardTotal(hand) + 10);
