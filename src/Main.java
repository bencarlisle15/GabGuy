...
public class Main extends JPanel implements KeyListener, FocusListener {
  ...
  private OutputAdd outAdd =
      new OutputAdd(mag, out, left, this, Thread.currentThread(), output);
  ...
  public Main() {
    ...
    try {
      ...
      out.insertString(out.getLength(),
                       "Hi I'm GabGuy, let's talk\nHit F1 for help.\n", left);
  ...
  public void create(Main m) {
    ...
    try {
      frame.setIconImage(
          ImageIO.read(getClass().getResource("files/chat.png")));
    } catch (IOException e) {
    }
  ...
  public void keyPressed(KeyEvent e) {
    ...
    // adds bash style commands for up and down
      ...
      // sets the input text to the (if available) the response below (visually
      // above) the current pos
      pos = Math.max(0, pos - 1);
      if (inputResponses.size() > pos && pos >= 0)
        input.setText(inputResponses.get(pos));
    } else if (e.getKeyCode() == KeyEvent.VK_DOWN) {
      // sets the input text to the (if available) the response above (viually
  ...
  public void keyReleased(KeyEvent e) {
    ...
    if (outAdd.getChecking()) {
      ...
      try {
        out.remove(out.getText(0, out.getLength()).lastIndexOf("working..."),
                   10);
      ...
      if (!help && e.getKeyCode() == KeyEvent.VK_F1)
        ...
        // tells the user that their process has stopped and removes the working
        // message
        try {
          out.remove(out.getText(0, out.getLength()).lastIndexOf("working..."),
                     10);
  ...
  public void focusGained(FocusEvent arg0) {
    ...
    if (input.getText().equals("Enter text here") ||
        input.getText().equals(
            "Words larger than 50 characters are not real and not allowed"))
      input.setText("");
    userError = false;
  }
