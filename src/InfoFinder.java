...
public class InfoFinder
{
	...
	{
		//sets the url to be checked as the beginning wiki url plus the String sent without the question or illegal characters
		line="https://en.wikipedia.org/wiki/"+fix(removeQuestion(l));
		//returns the response from the links page
		String ans=find();
		//if an error of any kind (except for no connection) occurred then continue the searching
		...
		if (!ask.substring(0,Math.min(20,ask.length())).equals("An error has occured"))
			return ask;
		//the beginning ask url + the String without the question or illegal characters
		line="http://www.ask.com/web?o=0&l=dir&qo=serpSearchTopBox&q="+fix(removeQuestion(l));
		//returns the response from the links page
		String noQuestion=askFind();
		//if an error of any kind (except for no connection) occurred then return the wiki response since it is more specific
	...
	{
		//removes the question if it has the word is or 's in it if not then the question shouldn't be removed anyway
		int pos=s.indexOf("'s ")+3;
		if (pos==2&&s.indexOf(" is ")<0)
			return s;
		else if (pos==2||s.indexOf(" is ")<s.indexOf("'s ")&&s.indexOf(" is ")>0)
			pos=s.indexOf(" is ")+4;
	...
	private String askFind()
	{
		try
		{
			...
			BufferedReader lr=new BufferedReader(new InputStreamReader(new URL(line).openConnection().getInputStream()));
			//if it's not ready then the connection must be bad and throws the appropriate error
			if (!lr.ready())
			...
			lr.close();
			//could not find the target text therefore the target block does not exist and the given question is too specific
